// ============================================================================
// Operations API — GET /api/v1/events/stream (Server-Sent Events)
// ============================================================================
...
export async function GET(request: NextRequest) {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      // 3. Subscribe to canonical events
      const eventsToSubscribe = [
        'ArtifactCreated', 'ArtifactUpdated', 'ArtifactDeleted', 'MetadataUpdated',
        'PreviewGenerated', 'IndexUpdated', 'ThumbnailGenerated',
        ...
        'MetricCollected', 'InfrastructureAlert'
      ];
      ...
      eventsToSubscribe.forEach((eventName) => {
        const subId = eventBus.subscribe(eventName, (event) => {
          try {
            ...
            console.error(`[SSE] Failed to write event "${eventName}" to stream:`, err);
          }
        });
        subIds.push(subId);
      });
      ...
      request.signal.addEventListener('abort', () => {
        ...
        console.log('[SSE] SSE connection closed and subscriptions cleaned up.');
      });
    }
  });
