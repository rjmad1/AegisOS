...
export class OktaProvider implements AuthProvider {
  ...
  constructor() {
    this.issuerUrl = process.env.OKTA_ISSUER || 'https://dev-example.okta.com/oauth2/default';
    this.clientId = process.env.OKTA_CLIENT_ID || 'okta_oauth_client_id_dev_secure';
    this.clientSecret = process.env.OKTA_CLIENT_SECRET || 'okta_oauth_client_secret_dev_secure';
  ...
  async handleCallback(url: URL, expectedState: string, expectedNonce: string): Promise<AuthUserProfile> {
    ...
    if (!claims) {
      throw new Error('Okta ID Token claims not found in response');
    }

    return {
      id: claims.sub,
      email: (claims.email as string) || 'okta-user@enterprise.local',
      name: (claims.name as string) || 'Okta Enterprise User',
    };
  }
}
