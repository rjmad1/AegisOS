...
export class KeycloakProvider implements AuthProvider {
  ...
  constructor() {
    this.issuerUrl = process.env.KEYCLOAK_ISSUER || 'http://localhost:8080/realms/master';
    this.clientId = process.env.KEYCLOAK_CLIENT_ID || 'keycloak_oauth_client_id_dev_secure';
    this.clientSecret = process.env.KEYCLOAK_CLIENT_SECRET || 'keycloak_oauth_client_secret_dev_secure';
  ...
  async handleCallback(url: URL, expectedState: string, expectedNonce: string): Promise<AuthUserProfile> {
    ...
    if (!claims) {
      throw new Error('Keycloak ID Token claims not found in response');
    }

    return {
      id: claims.sub,
      email: (claims.email as string) || 'keycloak-user@enterprise.local',
      name: (claims.name as string) || 'Keycloak Enterprise User',
    };
  }
}
