...
export class OpenClawRuntimeProvider implements IRuntimeProviderAdapter {
  id = "openclaw-runtime-provider";
  name = "OpenClaw Runtime Provider";
  ...
  async initialize(config: Record<string, any>): Promise<void> {
    ...
    console.log(`[OpenClawRuntimeProvider] Initialized. StateDir: ${this.stateDir}, ConfigPath: ${this.configPath}`);
    this.initialized = true;
  }

  async shutdown(): Promise<void> {
    console.log("[OpenClawRuntimeProvider] Shutting down.");
  }
  ...
  async checkHealth(): Promise<HealthCheckResult> {
    ...
    const dbPath = path.join(this.stateDir, "Metadata/state/openclaw.sqlite");
  ...
  async getRuntimeStatus(): Promise<any> {
    ...
    return {
      online: isPortActive,
      pid: isPortActive ? 8840 : undefined, // openclaw service standard PID from deployment manager
      port: 18789,
  ...
  async listActiveSessions(): Promise<any[]> {
    ...
    if (fs.existsSync(dbPath)) {
      try {
        ...
        console.error("[OpenClawRuntimeProvider] Failed to query sessions from SQLite:", err);
      }
    }
