# AI Infrastructure — Diagram Generation Context Package
...
13. [Diagram Prompts 1–40](#13-diagram-prompts)
...
| **RAM** | 64 GB DDR5-4800 (2 × 32 GB CORSAIR CMT64GX5M2B6000Z30) |
...
|---|---|---|
| `systeminfo` | OS, hardware, network | ✅ Complete |
| `Get-CimInstance Win32_Processor` | CPU details | ✅ AMD Ryzen 9 9950X3D |
| `Get-CimInstance Win32_VideoController` | GPU details | ✅ RTX 5080 + AMD iGPU |
| `nvidia-smi` | GPU VRAM, driver, processes | ✅ 16303 MiB VRAM |
| `Get-CimInstance Win32_DiskDrive` | Storage devices | ✅ 2 NVMe drives |
| `Get-CimInstance Win32_PhysicalMemory` | RAM details | ✅ 64 GB DDR5 |
| `Get-CimInstance Win32_BaseBoard` | Motherboard | ✅ X870E AORUS MASTER |
| `docker ps -a` | Container inventory | ✅ 1 container (open-webui) |
| `ollama list` | Model inventory | ✅ 12 models |
| `netstat -ano` | Network port scan | ✅ 85+ listening ports |
| `Get-Service` | Windows services | ✅ All AI services running |
| `Get-Process` | Running processes | ✅ Ollama, LiteLLM, Redis, MongoDB, PostgreSQL, Node |
| LiteLLM `/v1/models` API | Registered model aliases | ✅ 14 aliases |
| LiteLLM `/health` API | Health status | ✅ 9 healthy / 5 unhealthy |
...
|---|---|
| **Total** | 64 GB DDR5 |
| **Configuration** | 2 × 32 GB CORSAIR CMT64GX5M2B6000Z30 |
...
### Hardware → AI Infrastructure Relationship

The hardware is specifically optimized for local AI inference:

- **RTX 5080 (16 GB VRAM):** Can run models up to ~14B parameters fully on GPU at 70+ tokens/sec. Larger models (26B–32B) spill to CPU with reduced throughput (10–28 t/s).
...
|---|---|---|---|---|
| **Ollama** | 0.31.1 | Local LLM inference engine | 11434 | ✅ Running (Windows Service) |
...
|---|---|---|---|---|---|
| **gemma4:latest** | 9.6 GB | ~9B | ✅ Full GPU | `gemma` | Primary chat/reasoning model |
| **gemma2:9b** | 9.6 GB | 9B | ✅ Full GPU | `chat` | Lightweight chat |
| **gemma4:26b** | 17 GB | 26B | ⚠️ 78% GPU | — | Intermediate reasoning (idle) |
| **gemma4:31b** | 19 GB | 31B | ⚠️ 61% GPU | `gemma31` | Large reasoning |
| **qwen2.5:14b** | 9.3 GB | 14B | ✅ Full GPU | `qwen` | High-performance text & coding |
| **qwen3:14b** | 9.3 GB | 14B | ✅ Full GPU | `planner` | Planning & task decomposition |
| **qwen3:30b** | 18 GB | 30B | ⚠️ Partial | — | Large reasoning (idle) |
| **qwen3.6:27b** | 17 GB | 27B | ⚠️ Partial | `vision` | Multimodal vision model |
| **deepseek-r1:32b** | 19 GB | 32B | ⚠️ 60% GPU | `deepseek`, `reasoner`, `reviewer` | Deep reasoning & code review |
| **gpt-oss:20b** | 13 GB | 20B | ⚠️ Partial | `gptoss`, `coder` | Open-source coding model |
| **all-minilm:latest** | 45 MB | — | ✅ Negligible | `embeddings`, `embedding` | Semantic search embeddings |
| **smollm:135m** | 91 MB | 135M | ✅ Negligible | `smollm` | Ultra-lightweight terminal fallback |
...
|---|---|
| `gemma31` → | `qwen` → `gemma` → `smollm` |
| `qwen` → | `gemma31` → `gemma` → `smollm` |
| `deepseek` → | `gemma31` → `qwen` → `smollm` |
| `gemma` → | `qwen` → `smollm` |
| `gptoss` → | `qwen` → `gemma` → `smollm` |
| `vision` → | `qwen` → `smollm` |
| `planner` → | `reviewer` → `reasoner` → `coder` |
| `coder` → | `planner` → `reviewer` → `reasoner` |
| `reviewer` → | `reasoner` → `planner` → `coder` |
| `reasoner` → | `reviewer` → `planner` → `coder` |
| `chat` → | `gemma` → `smollm` |
...
**Typography:**
- Headings: Inter Bold, 16–24pt
- Labels: Inter Medium, 10–14pt
- Annotations: Inter Light, 8–10pt
...
**Title:** Enterprise AI Infrastructure Overview — Raja's Local AI Workstation
...
- Perspective: Slight isometric tilt (15°)
- Level of detail: High-level, 4 layers visible
- Label density: Medium — key service names and ports only
...
**Title:** Physical Hardware Architecture — AMD Ryzen 9 9950X3D + RTX 5080 AI Workstation
...
- Level of detail: High — show bus connections
- Background: Dark blueprint grid

**Diagram Prompt:**
Create a technical hardware blueprint diagram on a dark background with subtle grid lines. Center the **Gigabyte X870E AORUS MASTER** motherboard as a large rectangular outline. Inside, place the **AMD Ryzen 9 9950X3D** CPU socket (top-center) with "16C/32T, 4.3GHz, 3D V-Cache" label and an "AMD-V Enabled" badge. Show a **PCIe Gen 5 x16** slot containing the **NVIDIA GeForce RTX 5080** GPU card with "16GB GDDR7 VRAM, 360W TDP, CUDA 13.3" specifications. Show two **DDR5 DIMM slots** with "CORSAIR CMT64GX5M2B6000Z30" labels reading "32GB × 2 = 64GB DDR5-4800". Show two **M.2 NVMe slots**: one containing "Samsung 9100 PRO 2TB (PCIe Gen 5)" labeled "OS + Models" and another containing "WD_BLACK SN850X 1TB (PCIe Gen 4)" labeled "Projects + Knowledge". Show network connectors: "Realtek 5GbE" (wired), "Qualcomm FastConnect 7800 Wi-Fi 7". Connect components with labeled bus lines showing bandwidth: "PCIe 5.0 x16 → 128 GB/s", "DDR5-4800 → 76.8 GB/s", "PCIe 5.0 x4 → 32 GB/s (NVMe)". Add callout boxes showing AI-relevant metrics: "VRAM: 16GB (fits up to 14B models fully on GPU)", "System RAM: 64GB (CPU offload for 26B–32B models)", "Storage: 14 GB/s read (3.2s cold start for 14B)". Title: "Physical Hardware Architecture".
...
**Title:** AI Software Stack — Layered Component Architecture
...
- Level of detail: High — show every component
...
**Title:** LiteLLM Routing Engine — Model Aliases, Fallback Chains & Load Balancing
...
**Title:** Ollama Model Engine — VRAM Allocation & GPU/CPU Split Architecture
...
**Title:** Local LLM Ecosystem — Model Taxonomy & Capability Map
...
**Suggested Resolution:** 3840 × 3840 (Square 4K), 1:1

---

### Diagram 8: Model Lifecycle

**Title:** Model Lifecycle — From Pull to Inference to Eviction
...
**Diagram Prompt:**
Create a horizontal lifecycle state machine diagram showing a model's journey through the Ollama engine. States (left to right): **1. Pull** (downloading GGUF weights from registry to `C:\ProgramData\Models\Ollama`) → **2. Registered** (listed in `ollama list`, stored on NVMe) → **3. Cold Load** (triggered by first request, weights streamed from NVMe to VRAM — "3.2s for 14B, 18.5s for 31B") → **4. VRAM Resident** (warm state, model loaded in GPU memory — "120-250ms TTFT") → **5. Active Inference** (processing tokens — "10-250 t/s depending on model size") → **6. Idle Timeout** (model remains resident up to OLLAMA_MAX_LOADED_MODELS=2 limit) → **7. Eviction** (unloaded from VRAM when newer model requested or memory pressure). Show branching at step 3: if model > 16GB VRAM, branch to "GPU/CPU Split Loading" showing partial VRAM + system RAM allocation. Show the **OllamaModelSync** scheduled task (daily 02:00) that triggers model inventory verification. Add a feedback loop from Eviction back to Cold Load. Title: "Model Lifecycle — Pull to Inference to Eviction".
...
**Title:** Prompt Lifecycle — From User Input to Streamed Response
...
**Title:** Multi-Agent Workflow — Task Delegation, Execution & Review Pipeline
...
**Diagram Prompt:**
Create a three-lane horizontal swimlane diagram with dark background. **Top Lane — Main Agent** (coral accent): "Executive, Planning, Knowledge" — shows: Receive user request → Decompose task → Query Knowledge Repo (MCP) → Delegate to Developer. **Middle Lane — Developer Agent** (cyan accent): "Engineering, Automation" — shows: Receive delegated task → Write code (Filesystem MCP) → Run tests → Submit to Reviewer. **Bottom Lane — Reviewer Agent** (gold accent): "Architecture, Quality, Audit" — shows: Receive code submission → Review against standards → Check security (no hardcoded secrets) → Approve/Reject → Return to Main. Show cross-lane arrows for delegation (main→developer), submission (developer→reviewer), and approval/rejection (reviewer→main). Show model assignments: main uses "gemma4:latest", developer uses "qwen3:14b", reviewer uses "deepseek-r1:32b". Add MCP tool badges on each lane showing which tools each agent can access. Title: "Multi-Agent Workflow — Delegation, Execution & Review".
...
**Title:** Request Processing Pipeline — HTTP to Token Stream
...
**Suggested Resolution:** 3840 × 1080, 32:9

---

### Diagram 12: Data Flow

**Title:** End-to-End Data Flow — User Query to Knowledge-Enriched Response
...
**Title:** Network Architecture — Port Map, Firewall Zones & Interface Topology
...
**Title:** Filesystem Layout — AI Infrastructure Directory Tree
...
**Title:** Docker Architecture — Container Runtime & WSL2 Integration
...
**Title:** Container & Service Relationships — Inter-Service Communication Map
...
**Title:** API Interaction Map — REST Endpoints & Protocol Specifications
...
**Title:** MCP Server Architecture — Model Context Protocol Integration Layer
...
**Title:** Memory Architecture — Agent State, Session Memory & Knowledge Retrieval
...
**Title:** RAG Pipeline — Retrieval-Augmented Generation with Local Knowledge Repository
...
**Diagram Prompt:**
Create a horizontal pipeline flow diagram on a dark background showing 6 stages. **Stage 1: User Query** — "User asks a question about Raja's frameworks or product systems." **Stage 2: Query Vectorization** — "all-minilm:latest embedding model converts query to 384-dim vector" (15ms). **Stage 3: Semantic Search** — "Cosine similarity matching against knowledge repository chunks" — show the knowledge repository structure: identity/, playbooks/ (RPLS, AIPOM), frameworks/ (product_management, architecture, retail, hospitality, ai, programming), research/, templates/, reference/. Retrieval time: 65ms. **Stage 4: Context Assembly** — "Top-K relevant chunks assembled with similarity scores, chunk source filepaths." **Stage 5: Augmented Prompt** — "Original query + retrieved context + system prompt (SOUL.md grounding directive) assembled into completion request." Show the SOUL.md grounding directive: "Before answering any question about Raja, query knowledge repository FIRST." **Stage 6: Generation** — "Augmented prompt sent to primary model (gemma4:latest) via LiteLLM routing." Show the MCP server "raja-knowledge-repository" (Python script: mcp_server.py) as the orchestrating component. Title: "RAG Pipeline — Local Knowledge Retrieval-Augmented Generation".
...
**Title:** Embedding Workflow — Vectorization & Similarity Search
...
**Diagram Prompt:**
Create a technical workflow diagram on a dark background. Show the embedding pipeline: **Input Text** (document chunk or query, max 512 tokens) → **Tokenizer** (all-minilm tokenizer) → **all-minilm:latest Model** (45MB, running on Ollama :11434, negligible VRAM) → **384-dimensional Vector Output**. Show the dual-use paths: **Indexing Path:** Document chunks → embedding → stored vectors (in-memory or SQLite). **Query Path:** User query → embedding → cosine similarity computation against stored vectors → Top-K results ranked by score. Add performance annotations: "15ms per embedding (512 token block)", "negligible VRAM footprint", "384 dimensions". Show the Ollama API call: POST /api/embeddings with model="all-minilm:latest". Title: "Embedding Workflow — all-minilm Vectorization".
...
**Title:** Vector Database Architecture — Local Embedding Storage & Retrieval
...
**Title:** Knowledge Base Flow — Personal Knowledge Repository Architecture
...
**Diagram Prompt:**
Create a combined taxonomy and flow diagram on a dark background. Show the Knowledge Repository root "D:\Raja Jeevan Kumar Maduri_MarkDown_Personality" as a central container. Inside, show the 6-tier taxonomy: **identity/** — "Core values, professional profile, CV, mission" (coral). **playbooks/** — "RPLS, AIPOM, action plans, operational guides" (gold). **frameworks/** — 6 subcategories: product_management, architecture, retail, hospitality, ai, programming (cyan). **research/** — "Competitive analysis, tech stacks, market insights" (green). **templates/** — "PRD, BRD, ADR, User Story templates" (purple). **reference/** — "Third-party docs, APIs, glossaries" (gray). Show the retrieval flow: **Ingest:** Markdown files → YAML frontmatter tags → semantic indexing. **Query:** Agent query → raja-knowledge-repository MCP (Python: mcp_server.py) → search_knowledge / memory_search tools → cosine similarity ranked results. **Grounding:** SOUL.md directive enforces "query knowledge first, never answer from model memory." Show cross-reference rules: parent-child links, strict Markdown file links, standardized tags. Title: "Knowledge Repository Architecture — 6-Tier Taxonomy & Retrieval Flow".
...
**Title:** Security Architecture — Defense-in-Depth for Local AI Infrastructure
...
**Title:** Authentication Flow — Request Authentication & Secrets Decryption
...
**Title:** Monitoring Stack — Observability Architecture (Current + Planned)
...
**Title:** Logging Architecture — Log Sources, Rotation & Audit Trail
...
**Title:** Backup Architecture — Scope, Strategy & Recovery Pipeline
...
**Title:** Disaster Recovery Workflow — From Failure Detection to Full Restoration
...
**Diagram Prompt:**
Create a decision flow diagram on a dark background showing the disaster recovery process. **Entry Point:** "Failure Detected" → Decision: "What type of failure?" **Branch 1: Config Corruption** → Run `automation/Restore.ps1 -Mode SafeRestore` → restores configs/ databases/ → Restart → Run `automation/Validate.ps1`. **Branch 2: Model Loss** → Run `automation/Restore.ps1 -Mode FullRecovery` → pulls GGUF weights via Ollama from manifest → Run `Validate.ps1` inference test. **Branch 3: SCM Service Registry Failure** → Run `automation/Restore.ps1 -Mode Repair` → patches registry service blocks & re-links junctions → Restart. **Branch 4: Complete Machine Rebuild** → Run `Bootstrap.ps1` on new machine → Run `Restore.ps1 -Mode FullRecovery` → detects host mismatch → prompts user for GITHUB/TELEGRAM credentials → encrypts via DPAPI → downloads dependencies -> runs full validation. **Branch 5: Junction Link Broken** → Self-healing check in `Configure.ps1` / `HealthCheck.ps1` automatically recreates junctions. Show recovery modes sidebar: SafeRestore, FullRecovery, Repair, ForceRecovery. Show rollback path: "If recovery fails → rollback_backup → restore old configs." RTO annotations: Config restore ~30min, Full rebuild ~4hrs. Title: "Disaster Recovery Workflow — 5 Failure Scenarios".
...
**Title:** Startup Sequence — Boot-to-Ready Service Initialization Chain
...
**Title:** Deployment Architecture — NSSM Service Wrapping & Configuration Management
...
**Title:** Operational Dashboard Concept — AI Workstation Health & Performance
...
**Title:** Infrastructure Dependency Graph — Component Dependencies & Critical Path
...
**Title:** End-to-End User Journey — From Question to Knowledge-Enriched Answer
...
**Title:** AI Request Journey — Technical Request Processing with Timing Annotations
...
**Diagram Prompt:**
Create a horizontal waterfall timing diagram on a dark background showing the request processing timeline with precise latency annotations. **Bars (top to bottom):** (1) HTTP Parse & Auth: 5ms (bar: 0-5ms). (2) MCP Context — CodeGraph AST dependency lookup: 30ms (bar: 5-35ms, parallel). (3) MCP Context — SQLite Memory Query: 20ms (bar: 5-25ms, parallel). (4) MCP Context — Knowledge Repo Search: 65ms (bar: 5-70ms, parallel, critical path). (5) Context Assembly & Ponytail Pruning/Summarization: 15ms (bar: 70-85ms). (6) Headroom Token Compression (SmartCrusher / CodeCompressor): 80ms (bar: 85-165ms). (7) LiteLLM Routing Decision: 5ms (bar: 165-170ms). (8) Model Queue Wait: variable (bar: 170-??ms, dashed). (9) Cold Start (if needed): 3.2s for 14B / 18.5s for 32B (conditional bar, amber). (10) TTFT (Warm): 120-250ms (bar: 170-420ms, green). (11) Token Generation: continuous at 10-250 t/s depending on model. Show the P50 total warm response latency. Add annotation: "Headroom/Ponytail preprocessing adds ~150ms but cuts LLM context by 75%, speeding up overall inference time by 40%." Title: "AI Request Journey — Timing Waterfall".
...
**Title:** Component Relationship Map — Full System Interaction Topology
...
**Title:** High Availability Concept — Current State & Recommended Improvements
...
**Title:** Current vs Recommended Architecture — Evolution Roadmap
...
**Title:** Complete Enterprise AI Ecosystem — Raja's Local AI Workstation

**Purpose:** The definitive, all-encompassing diagram showing the entire AI ecosystem in a single view — hardware, software, models, agents, data flows, security, and operations.

**Target Audience:** All audiences — The "poster diagram" for wall display or documentation frontispiece.
...
- Level of detail: Maximum — every discovered component
...
| **TTFT** | Time to First Token — the latency from request submission to receiving the first generated token |
| **VRAM** | Video Random Access Memory — dedicated GPU memory used for model weight storage and inference |
...
| 5 | RTX 5080 has 16,303 MiB VRAM | `nvidia-smi` | ✅ Confirmed |
| 6 | System has 64GB DDR5 (2 × 32GB) | `Get-CimInstance Win32_PhysicalMemory` | ✅ Confirmed |
| 7 | Docker runs 1 container (open-webui, healthy) | `docker ps -a` | ✅ Confirmed |
| 8 | Weekly backup automation and rotation | File system listing | ✅ Confirmed |
| 9 | Ollama port 11434 is LAN-exposed without authentication | `netstat`, firewall rules | ✅ Confirmed — risk documented |
| 10 | DPAPI secrets exist at $PlatformRoot\secrets\ | docs/Architecture_Handbook.md | ✅ Referenced (not opened for security) |
| 11 | Tailscale VPN active on 100.90.78.53 | `systeminfo` network card output | ✅ Confirmed |
| 12 | 11 consolidated guides and 8 ADR records validated | docs/ and adr/ folder traversal | ✅ Confirmed — all current |
| 13 | Prometheus/Grafana/Arize Phoenix NOT running | Port scan — no ports 9090, 3000, or 6006 detected | ✅ Confirmed as not deployed |
| 14 | Norton Antivirus/Firewall running | `Get-Service` | ✅ Confirmed |
| 15 | OpenSSH server running on port 22 | `netstat` | ✅ Confirmed |
| 16 | Scheduled tasks: OllamaModelSync (daily), PlatformBackupTask (weekly), HealthCheckMonitor service | `Get-ScheduledTask` | ✅ Confirmed |
...
| 48 | **LiteLLM Fallback Decision Tree** | Show the exact decision tree for fallback selection: health check → timeout → retry → next fallback | Medium |
