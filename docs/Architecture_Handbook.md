...
graph TD
    ...
    subgraph API & Gateway Control Layer
        AegisOS[AegisOS AI Gateway :18789]
    ...
    Console -->|API Requests| AegisOS
    WebUI -->|API Requests| LiteLLM
    IDE -->|Tool Execution| AegisOS
    
    AegisOS -->|Prompt Proxy| LiteLLM
    AegisOS <-->|Fetch Context| MCP
...
*   **Sub-system 1.1: Web Administration Console (Next.js 16 App)**
    ...
    *   **Component 1.1.2: State Stores (Zustand):** Managed React state (e.g., [appStore](file:///d:/1_Projects/AegisOS/src/store/appStore.ts) and [authStore](file:///d:/1_Projects/AegisOS/src/store/authStore.ts)) storing environment profiles, loaded models, and active background tasks.
...
*   **Sub-system 2.1: AegisOS AI Gateway (Port 18789)**
...
*   **Sub-system 4.1: Background Job Queue**
    *   **Component 4.1.1: Queue Manager ([job-queue.ts](file:///d:/1_Projects/AegisOS/src/infrastructure/jobs/job-queue.ts)):** Manages scheduling, prioritization, and locking of long-running operations.
...
*   **Sub-system 4.2: Event Bus System**
    *   **Component 4.2.1: Event Broker ([event-bus.ts](file:///d:/1_Projects/AegisOS/src/infrastructure/events/event-bus.ts)):** Distributes events using a publish-subscribe pattern, enabling decoupled modules to react to system changes.
...
sequenceDiagram
    ...
    participant Gateway as AegisOS Gateway (:18789)
...
graph TD
    ...
    Secrets -->|DPAPI Encrypted| EncryptedSecrets[AegisOS_secrets.enc]
    Logs -->|Rotated Logs| ServiceLogs[AegisOSService.log]
...
2.  **`/configs/`**: Contains configuration files (YAML, JSON, XML) generated by [Configure.ps1](file:///d:/1_Projects/AegisOS/automation/Configure.ps1).
